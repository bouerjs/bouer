...
import indent from 'js-beautify';
import { builds, version } from './config.js';
    ...
    watcher.on('event', event => {
      ...
      switch (event.code) {
        case 'BUNDLE_START':
          ...
          console.info(
            `${prefix} ⚒️  Generating file -> ${event.output[0]}`
          );
          break;
          ...
          bundle.generate(output)
            ...
            .finally(() => {
              ...
              if (buildKeys.length == numberOfGenerations) {
                ...
                console.log(
                  `${prefix} ⏳ Waiting for a file to change to rebuild...`
                );
              }
            });
          break;
...
function writeFile(dest, code, zip) {
  return new Promise((resolve, reject) => {
    function report(extra) {
      ...
      const colored =
        `\x1b[1m\x1b[34m${content}\x1b[39m\x1b[22m`;
